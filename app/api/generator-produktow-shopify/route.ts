...
import { crawlBatrea } from "../../../lib/batreaCrawler.server";
...
const productSchema = z.object({
  ...
  seo: z.object({ titleTag: z.string(), descriptionTag: z.string(), handle: z.string() }), schemaProduct: z.string(), schemaFaq: z.string(), gaps: z.array(z.string()),
});
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const crawled = await crawlBatrea(details.urls);
    if (!crawled.length) throw new Error("Crawler nie znalazł dozwolonych stron produktów.");
    const sourceInput = `Nadpisanie modelu: ${details.overrideName || "brak"}\nDane techniczne wklejone:\n${details.technicalText || "brak"}\nPDF użytkownika:\n${pdfs.join("\n\n") || "Brak PDF"}\n\nETAP 1 — DANE ŹRÓDŁOWE BATREA (jedyna podstawa Etapu 2):\n${JSON.stringify(crawled).slice(0, 100_000)}`;
    const result = await generateText({ model: google("gemini-3.1-flash-lite"), system: `${generationSystem}\nDodaj schemaProduct jako JSON-LD Product i schemaFaq jako JSON-LD FAQPage (pusty obiekt, gdy brak FAQ).`, prompt: sourceInput, output: Output.object({ schema: matrixifySchema }), maxRetries: 1, temperature: .1 });
