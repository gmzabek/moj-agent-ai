...
export async function POST(request: Request) {
  ...
  try {
    ...
    const result = await generateText({ model: google("gemini-3.1-flash-lite"), system, prompt: `URL-e:\n${details.urls.join("\n")}\nNadpisanie modelu: ${details.overrideName || "brak"}\nDane techniczne wklejone:\n${details.technicalText || "brak"}\n${pdfs.join("\n\n") || "Brak PDF"}`, output: Output.object({ schema: matrixifySchema }), tools: { readWebPage: readWebPageTool, calculator: calculatorTool, ...(enabledSearch ? { google_search: google.tools.googleSearch({}) } : {}) }, stopWhen: stepCountIs(24), temperature: .1 });
