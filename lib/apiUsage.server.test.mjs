...
test("domyślny limit wynosi 10k tokenów", () => {
  assert.equal(DEFAULT_DAILY_TOKEN_LIMIT, 10_000);
});
...
test("blokuje kolejne wywołanie po wykorzystaniu 10k tokenów", async () => {
  const supabase = {
    rpc: async () => ({ data: "10000", error: null }),
  };
...
test("dopuszcza wywołanie poniżej dziennego limitu", async () => {
  const supabase = {
    rpc: async () => ({ data: 9_999, error: null }),
  };
